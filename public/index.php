...
foreach ($listaContas as $conta) {
    ...
    if (isset($totaisPorConta_Geral[$conta['id']])) {
        $mov = $totaisPorConta_Geral[$conta['id']];
        $saldo = $saldo + $mov['entrada'] - $mov['saida'];
    }
...
foreach ($listaContas as $conta) {
    $contaId = $conta['id'];
    $saldo = $conta['saldo_inicial']; 
    if (isset($totaisPorConta_Tabela[$contaId])) {
        $movimentacoes = $totaisPorConta_Tabela[$contaId];
    ...
    <div class="flex flex-wrap justify-between items-center p-6">
        <div>
            ...
            <div class="mt-2 text-sm">
                ...
                ?>
                <a href="<?php echo $link_geral; ?>" class="<?php echo $classe_geral; ?>">Geral (Consolidado)</a>
                <span class="mx-2 text-gray-400">|</span>
                <a href="<?php echo $link_mes; ?>" class="<?php echo $classe_mes; ?>">No Mês Selecionado</a>
            </div>
        </div>
        <div class="text-right mt-4 md:mt-0">
             <h3 class="text-sm font-medium text-gray-500 uppercase tracking-wider">Patrimônio / Saldo Total (<?php echo $visao_saldo; ?>)</h3>
             <p class="text-2xl font-semibold <?php echo $saldoGeralTabela >= 0 ? 'text-gray-900' : 'text-red-600'; ?>">
                R$ <?php echo number_format($saldoGeralTabela, 2, ',', '.'); ?>
             </p>
        </div>
    </div>
            ...
            <?php else: ?>
                <?php foreach ($contasComSaldo as $conta): ?>
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-semibold <?php echo $conta['saldo_atual_calculado'] >= 0 ? 'text-gray-900' : 'text-red-600'; ?>">
